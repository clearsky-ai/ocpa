...
def get_hidden_transitions_to_enable(marking, places_with_missing, places_shortest_path_by_hidden):
    ...
    places_with_missing_keys = sorted(
        places_with_missing_keys, key=lambda x: x.name)
    for p1 in marking_places:
        for p2 in places_with_missing_keys:
            if p1 in places_shortest_path_by_hidden and p2 in places_shortest_path_by_hidden[p1]:
                hidden_transitions_to_enable.append(
                    places_shortest_path_by_hidden[p1][p2])
...
def enable_hidden_transitions(net, marking, activated_transitions, visited_transitions, all_visited_markings,
    ...
    for z in range(10000000):
        ...
        for k in range(j_indexes[z % len(hidden_transitions_to_enable)], len(
            ...
            j_indexes[z % len(hidden_transitions_to_enable)] = j_indexes[z % len(
                hidden_transitions_to_enable)] + 1
...
def apply_trace(trace, net, initial_marking, final_marking, trans_map, enable_pltr_fitness, place_fitness,
    ...
    for i in range(len(trace)):
        if enable_postfix_cache and (str(trace_activities) in post_fix_caching.cache and
            ...
            trans_to_act = post_fix_caching.cache[str(
                trace_activities)][hash(marking)]["trans_to_activate"]
            for z in range(len(trans_to_act)):
                t = trans_to_act[z]
                act_trans.append(t)
            used_postfix_cache = True
            marking = post_fix_caching.cache[str(
                trace_activities)][hash(marking)]["final_marking"]
            break
        else:
            ...
            if enable_marktoact_cache and (hash(marking) in marking_to_activity_caching.cache and
                                           trace[i][activity_key] in marking_to_activity_caching.cache[hash(
                                               marking)]
                                           and trace[i - 1][activity_key] ==
                                           marking_to_activity_caching.cache[hash(
                                               marking)][trace[i][activity_key]]
                                           ["previousActivity"]):
                ...
                if trace[i][activity_key] in trans_map:
                        ...
                        if enable_pltr_fitness:
                            for place in tokens_added.keys():
                                if place in place_fitness:
                                    place_fitness[place]["underfed_traces"].add(
                                        trace)
                    ...
                    if not is_initially_enabled and cleaning_token_flood:
                        ...
                        new_marking_names_diff = [
                            x for x in new_marking_names if x not in old_marking_names]
                        new_marking_names_inte = [
                            x for x in new_marking_names if x in old_marking_names]
                        for p1 in new_marking_names_inte:
                            for p2 in new_marking_names_diff:
                                for comp in s_components:
                                    if p1 in comp and p2 in comp:
                                        place_to_delete = [place for place in list(
                                            marking.keys()) if place.name == p1]
                    ...
                    notexisting_activities_in_model[trace[i]
                                                    [activity_key]][trace] = current_event_map
                ...
                for place in marking:
                    if place in places_shortest_path_by_hidden and sink_place in places_shortest_path_by_hidden[place]:
                        connections_to_sink.append(
                            [place, places_shortest_path_by_hidden[place][sink_place]])
...
class ApplyTraceTokenReplay:
    ...
    def run(self):
        ...
        """
        self.t_fit, self.t_value, self.act_trans, self.trans_probl, self.reached_marking, self.enabled_trans_in_mark, self.missing, self.consumed, self.remaining, self.produced, self.tvs, self.ocs = \
...
def get_variants_from_log(log, activity_key, disable_variants=False):
    ...
    variants = variants_module.get_variants(
...
def run_timed_replay(log: EventLog, net: PetriNet, initial_marking: Marking, final_marking: Marking, parameters: Optional[Dict[Union[str, Parameters], Any]] = None) -> typing.ListAlignments:
    ...
    return_names = exec_utils.get_param_value(
        Parameters.RETURN_NAMES, parameters, False)
    ...
    variants = exec_utils.get_param_value(
        Parameters.VARIANTS, parameters, None)
    consider_activities_not_in_model_in_fitness = exec_utils.get_param_value(
        Parameters.CONSIDER_ACTIVITIES_NOT_IN_MODEL_IN_FITNESS, parameters, False)
...
def get_diagnostics_dataframe(log: EventLog, tbr_output: typing.ListAlignments, parameters: Optional[Dict[Union[str, Parameters], Any]] = None) -> pd.DataFrame:
    ...
    for index in range(len(log)):
        ...
        diagn_stream.append({"case_id": case_id, "is_fit": is_fit, "trace_fitness": trace_fitness,
